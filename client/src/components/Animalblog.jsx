...
const Animalblog = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axios.get('http://localhost:3333/blog/category/animal', {
  ...
  return (
    <div className="bg-gray-900 text-white p-6">
      <h1 className="text-3xl font-bold mb-6">Animal Blogs</h1>

      <div className="flex overflow-x-auto space-x-4 pb-4">
        {blogs.map((item, index) => (
          <Link key={item._id} to={`/singleblog/${item._id}`}>
          <div
            key={index}
